...
try:
	...
	if getEjecutar():
		...
		for clientesid,clientesip,clientesmac,clientespla,clientesnom,ipsfijasip,ipsfijasint,ipsfijasest,clientespro,clientessal,clientesdes,canalessub,canalesbaj in clientes:
			...
			#sql="""select canal,subida,bajada,planes.nombre from planes inner join canales on planes.canal=canales.id where planes.id=%d and planes.dia=%s and planes.hora=%s""" % (plan,dia,hora)
			#curs.execute(sql)
			#marca=curs.fetchall()
			#i=0
			#i = i+1
